...
__global__ void sum(float *d_a, float *d_b, float *d_d) {
    ...
    if(col < %(MATRIX_SIZE)s && row < %(MATRIX_SIZE)s)
        d_d[col * %(MATRIX_SIZE)s + row] = d_a[col * %(MATRIX_SIZE)s + row] + d_b[col * %(MATRIX_SIZE)s + row];
}
"""
...
add = mod.get_function("sum") #sum is a python keyword, so named add instead
...
add(device_a, device_b, device_d,
    block = (WIDTH, WIDTH, 1),
    grid = (WIDTH//2, WIDTH//2))
